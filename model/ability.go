...
// GetChannelExcluding is the non-memory-cache counterpart of
// GetRandomSatisfiedChannelExcluding. Capacity exclusions must behave the same
// regardless of MEMORY_CACHE_ENABLED.
func GetChannelExcluding(group string, model string, retry int, requestPath string, excluded map[int]struct{}) (*Channel, error) {
	...
	// Request-path and capacity filtering must happen before selecting a
	// priority. Otherwise, excluding the only channel at the highest priority
	// leaves an empty result and prevents fallback to the next priority.
	...
	if selectedChannelId == 0 {
		return nil, fmt.Errorf("channel not found for group %s, model %s, priority %d", group, model, targetPriority)
	}
