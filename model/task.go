...
type TaskPrivateData struct {
	...
	ChannelKeyId   int64  `json:"channel_key_id,omitempty"`  // 稳定 Key ID，用于异步退款/差额结算
	// ChannelKeyQuotaTracked records whether submission created and committed
	// a per-key quota reservation. RPM-only/unlimited keys still have an ID but
	// must not receive later refund adjustments against a zero usage balance.
	ChannelKeyQuotaTracked bool                `json:"channel_key_quota_tracked,omitempty"`
...
func InitTask(platform constant.TaskPlatform, relayInfo *commonRelay.RelayInfo) *Task {
	...
	if relayInfo != nil && relayInfo.ChannelMeta != nil {
		privateData.ChannelKeyId = relayInfo.ChannelMeta.ChannelKeyId
		privateData.ChannelKeyQuotaTracked = relayInfo.ChannelKeyQuotaReservationId > 0
